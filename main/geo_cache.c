...
#define GEO_CACHE_KEY_ATTEMPT    "attempt_done"
...
esp_err_t geo_cache_load(geo_cache_state_t *state)
{
    ...
    uint8_t attempt_done = 0;
    err = nvs_get_u8(handle, GEO_CACHE_KEY_ATTEMPT, &attempt_done);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW(TAG, "No se pudo leer attempt_done: %s", esp_err_to_name(err));
    }
    ...
    state->geo_attempt_done_today = (attempt_done != 0);
                                    ...
                                    bool geo_rate_limited_today)
{
    ...
    uint8_t new_attempt = geo_attempt_done_today ? 1 : 0;
    uint8_t new_rate_limit = geo_rate_limited_today ? 1 : 0;

    if (current_attempt == new_attempt && current_rate_limit == new_rate_limit) {
        nvs_close(handle);
        return ESP_OK;
    }
...
{
    return geo_cache_set_daily_flags(false, false);
}
